...
class HorizontalScrollLayout: UICollectionViewFlowLayout {
    ...
    let cellPaddingOffset : CGFloat = 100.0
    ...
    override func prepare() {
        ...
        if let collection = collectionView  {
            let collectionSize = collection.frame.size
            let calculatingSize = CGSize.init(width: collectionSize.width - cellPaddingOffset,
                                              height: collectionSize.height - 400)
            self.itemSize = calculatingSize
            
            collection.isPagingEnabled = true
            
            
            self.minimumLineSpacing = (cellPaddingOffset / 2.0 )
            
            self.sectionInset = UIEdgeInsets(top: 50,
